...
const marks10 = [10, -10, 0, 0.99999, 10.0000001, 3.14, "labas", true, false, [], 
NaN, Infinity, -Infinity, undefined, marksAverage];


function marksAverage(marks) {   
    if (marks.length === 0) {
        return 'Truksta duomenu (pazymiui)';
    }  

    let count = 0;
    let sum = 0;
                             /// console.log !!!!
    //console.log(marks);    /// visada patikrinti ar gavo ka reikia funkcija
    
    for (let i = 0; i < marks.length; i++) {
      const mark = marks[i];

      if (typeof mark === 'number' && mark >= 1 && mark <= 10 && mark % 1 === 0) {
      //if (mark > 0 && mark < 11 && Number.isInteger(mark)) {
        console.log(mark);
        sum += marks[i];
        count++;
...
console.log(average7, "-->", 'Truksta duomenu');
//console.log(average7, "-->", 'Vidurkiui skaiciuoti reikia bent vieno pazymio');
